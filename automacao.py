"""
FA Law — Automação de Gmail com IA (núcleo)
============================================
Classifica emails recebidos, aplica marcadores automáticos
e notifica no Google Chat quando há urgência.
...
# "NomeDetectadoPelaIA": "FA Law/Clientes/NomeDoMarcador",
CLIENTES_CONHECIDOS = {
...
MARCADORES = {
    "Consultivo":      "FA Law/Consultivo",
    "Audiencias":      "FA Law/Audiências",
    "Propaganda":      "FA Law/Propaganda",
    "OutrosClientes":  "FA Law/Clientes/Outros Clientes",
    "Outro":           "FA Law/Outros",
    "URGENTE":         "FA Law/⚠️ URGENTE",
}
...
def buscar_emails_nao_lidos(service) -> list:
    ...
    query = f'is:unread after:{timestamp} -label:"FA Law"'
...
def classificar_email(email: dict) -> dict:
    ...
    prompt = f"""Você é um assistente jurídico de um escritório de direito do trabalho brasileiro.
...
3. CLIENTE: se o email for de/sobre um destes clientes: {clientes_lista}
   → categoria "Cliente" e preencha o campo "cliente" com o nome EXATO da lista
...
# ──────────────────────────────────────────────
def notificar_google_chat(user_email: str, email: dict, classificacao: dict):
    """Envia notificação no Google Chat para emails urgentes."""
    webhook = CONFIG.get("GOOGLE_CHAT_WEBHOOK", "")
    if not webhook:
        log.warning("Webhook do Google Chat não configurado.")
        return

    motivo = classificacao.get("motivo_urgencia") or "Conteúdo sensível detectado"
    cliente = classificacao.get("cliente") or "não identificado"
    resumo = classificacao.get("resumo") or ""

    mensagem = {
        "text": (
            f"🚨 *EMAIL URGENTE — FA Law*\n\n"
            f"*Destinatário:* {user_email}\n"
            f"*De:* {email.get('de', '')}\n"
            f"*Assunto:* {email.get('assunto', '')}\n"
            f"*Cliente:* {cliente}\n"
            f"*Motivo:* {motivo}\n"
            f"*Resumo:* {resumo}\n"
            f"*Recebido:* {email.get('data', '')}"
        )
    }

    try:
        response = httpx.post(webhook, json=mensagem, timeout=10)
        if response.status_code == 200:
            log.info("  Notificação enviada no Google Chat.")
        else:
            log.warning(f"  Falha ao notificar Chat: {response.status_code}")
    except Exception as e:
        log.error(f"Erro ao notificar Google Chat: {e}")
    ...
    try:
        ...
        for msg_ref in emails:
            ...
            if classificacao.get("urgente"):
                stats["urgentes"] += 1
                notificar_google_chat(user_email, email, classificacao)
